...
from confopt.dataset.data import CUTOUT
...
class ConfigurableTrainerSingleLevel:
    def __init__(
        self,
        model: SearchSpace,
        data: AbstractData,
    ...
    def train(self, profile: Profile, epochs: int, is_wandb_log: bool = True) -> None:
        ...
        valid_data = dsets.CIFAR10(
            root="datasets", train=False, download=True, transform=valid_transform
        )
    ...
    def train_func(
        ...
        valid_loader: DataLoaderType,  # noqa: ARG002  TODO:Fix
        ...
        for step, (base_inputs, base_targets) in enumerate(train_loader):
            ...
            #     resource_lambda = 1.5e-3

            #     resource_penalty = (penalty[26]) / 6 + ratio * (penalty[25]) / 2
            #     log_resource_penalty = (penalty[37]) / 6 + ratio * (penalty[38]) / 2

            #     resource_loss = resource_lambda * resource_penalty
...
def data_transforms_cifar10(
    cutout: bool = False, cutout_length: int = 0
