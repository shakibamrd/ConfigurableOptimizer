...
def read_args() -> argparse.Namespace:
    ...
    # This does not matter, but would need for initialization
    parser.add_argument(
        "--lora-warm-epochs",
        default=0,
    ...
    parser.add_argument(
        "--supernet-path",
        help="Path for the supernet folder to load",
        type=str,
    )

    parser.add_argument(
        "--supernet-runtime",
        help="runtime appended after the path",
        type=str,
    )
    ...
    parser.add_argument(
        "--model_source",
        default="supernet",
        help="choose between supernet and arch_selection",
        type=str,
    )
...
def get_configuration(
    ...
    profile = profile_type(
        ...
        pt_select_architecture=True,
    )
    profile.configure_pt_selection(projection_interval=10)
    return profile
...
if __name__ == "__main__":
    ...
    exp_type = f"{args.searchspace}-{args.sampler}{lora_str}-{args.dataset}-Tuning"
    ...
    experiment_name = f"{args.sampler}-Tuning"
    ...
    experiment = Experiment(
        ...
        runtime=args.supernet_runtime,
    )

    config = profile.get_config()
    wandb.init(  # type: ignore
        name=profile.get_name_wandb_run(),
        project=config.get("project_name", "Configurable_Optimizer"),
        config=config,
    )

    experiment.select_perturbation_based_arch(
        profile=profile,
        model_source=args.model_source,
        load_saved_model=True,
        src_folder_path=args.supernet_path,
    )
