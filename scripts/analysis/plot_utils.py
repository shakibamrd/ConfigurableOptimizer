...
def plot_arch_values_by_edges(
    ...
    edges: Any = range(14),
    title: str = "",
    cell_types: Any = ("normal", "reduce"),
    ignore_ops: list[int] | None = None,
) -> None:
        ...
        for edge_idx in edges:
            ...
            if ignore_ops:
                for op in ignore_ops:
                    arch_values_mean_df = arch_values_mean_df.drop(
                        arch_values_mean_df.filter(like=f"_op_{op}").columns, axis=1
                    )
                    arch_values_std_df = arch_values_std_df.drop(
                        arch_values_std_df.filter(like=f"_op_{op}").columns, axis=1
                    )
...
def plot_gradient_matching_scores(
    mean_df: pd.DataFrame, std_df: pd.DataFrame, meta_info: str
) -> None:
    gm_scores_mean_df = get_mean_gradient_matching_score_df(mean_df)
    gm_scores_std_df = get_mean_gradient_matching_score_df(std_df)
    plot_line_chart_with_std_dev(gm_scores_mean_df, gm_scores_std_df, meta_info)
...
def plot_layer_alignment_scores(
    mean_df: pd.DataFrame,
    std_df: pd.DataFrame,
    meta_info: str,
    cell_types: tuple = ("normal", "reduce"),
) -> None:
    for cell_type in cell_types:
        mean_layer_alignment_df = get_layer_alignment_scores_all_cells_df(
            mean_df, cell_type
...
def plot_layer_alignment_scores_first_and_last(
    mean_df: pd.DataFrame,
    std_df: pd.DataFrame,
    meta_info: str,
    cell_types: tuple = ("normal", "reduce"),
