...
class TestConfTrainer(unittest.TestCase):
    def test_darts(self) -> None:
        ...
        m_optimizer = optim.SGD(
            searchspace.arch_parameters, lr=1e-3, momentum=0.0, weight_decay=0.1
        )
        a_optimizer = optim.Adam(
            searchspace.arch_parameters, lr=1e-3, betas=(0.9, 0.99), weight_decay=0.1
        )
        lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(m_optimizer, T_max=epochs)
        criterion = nn.CrossEntropyLoss()
        data = CIFAR10Data("datasets", 0, 0.5)
        logger = prepare_logger(save_dir="logs", seed=0, exp_name="Test")
